...
check_and_install_ollama() {
    if ! command -v ollama &> /dev/null; then
        # 显示安装提示
        dialog --infobox "检测到Ollama未安装，正在安装中..." 5 50
        ...
        if [ $success -eq 1 ]; then
            dialog --msgbox "✅ Ollama安装成功！\n\n请重启终端或运行: source ~/.bashrc" 8 50
        else
            dialog --msgbox "❌ Ollama安装失败！\n\n错误日志:\n$(tail -n 10 "$log_file")\n\n请尝试手动安装: curl https://ollama.ai/install.sh | sh" 15 60
            rm "$log_file"
            exit 1
        fi
